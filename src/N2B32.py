...
SK_4 = [[0,0,0,3,3,3,3,1,0,0,0,2,3,2,1,1],[0,0,0,2,3,2,1,1,0,0,0,2,3,2,1,1],[0,0,0,2,3,2,1,1,0,0,0,1,2,3,1,1]] # Matriz con KMERS
#SK_4 = [[3,3,3,3],[2,3,2,1],[1,2,3,1],[1,2,3,1],[0,1,2,3]] # Matriz con KMERS
h_SK_4 =np.ndarray((len(SK_4), len(SK_4[0]))).astype(np.uint8)
h_SK_4[:] = SK_4

h_TMP =np.ndarray((len(SK_4), len(SK_4[0]))).astype(np.uint32)


# TODO: Numpy, anadir columnas 0, tener el k de entrada de salida.
# Si cols de SK_4 no es multiplo de 4

# h_SK_2_32 Storage for output
h_SK_2_32=np.ndarray((h_SK_4.shape[0], h_SK_4.shape[1]/16)).astype(np.uint32)
...
N2B32.set_scalar_arg_dtypes([None, None, None, np.uint32, np.uint32])
...
N2B32(cola, rango_global, None, d_SK_4, d_TMP, d_SK_2_32, K, S)
cola.finish()
# Traigo datos
